...
<?php
   ...
   if($seConecto == true)
   {
      ...
      $consultaLista = $manejadorConsultas->armarConsultaListado("Empleado");
      //Obtenemos el cursor mediante el ADO a través de la consulta
      $cursorEmpleados = $controladorDAOMysql->realizarConsultaDatos($conexion,$consultaLista);
      //var_dump($cursorEmpleados);
      //Descargamos los datos al vector para la vista
      $datosConsulta = $manejadorConsultas->armarSalidaTablaEmpleados($cursorEmpleados);
      /*********************************************************************/
      /*********************************************************************/
      $totalSueldos = $manejadorConsultas->generarSumatoriaValores($datosConsulta,'salario');
      /*********************************************************************/
      /*********************************************************************/
   }
   else
...
<p><strong>Professor: Architect Jaime Alberto Gutiérrez Mejía. <?php echo date("Y"); ?></strong></p>
    ...
    <center>
      <table id="tabla" cellpadding="0" cellspacing="0" border="1">
        <tr>
            <td align="center"><b>CODIGO EMPLEADO</b></td>
            <td align="center"><b>NOMBRE EMPLEADO</b></td>
            <td align="center"><b>APELLIDO EMPLEADO</b></td>
            <td align="center"><b>SALARIO EMPLEADO</b></td>
        </tr>
    <?php
      for ($i = 0;  $i < count($datosConsulta);  $i+=4)
      {
    ?>
        <tr>
            <td><?=$datosConsulta[$i]['id']?></td>
            <td><?=$datosConsulta[$i+1]['nombre']?></td>
            <td><?=$datosConsulta[$i+2]['apellido']?></td>
            <td align="right">$  <?=number_format($datosConsulta[$i+3]['salario'],2)?></td>
        </tr>
    <?php
      }
  ?>
  <tr>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
    <td><b>TOTAL DE SALARIOS:</b></td>
    <td align="right"><b>$  <?=number_format($totalSueldos,2)?></b></td>
  </tr>
      </table>
    <br/>
    <br/>
